...
router.post("/", auth, (req, res) => {
  upload(req, res, async (err) => {
    ...
    axios
      ...
      .catch((err) => {
        let systemTestcases = [];
        const testarray= req.files.testcaseFile;
        testarray.forEach((file, index) => {
          console.log(file)
          let tc = file.buffer.toString();
          console.log(tc)
          let tl = tc.split("\n")
          for(let i=0;i<tl.length;i++){
            tl[i]=tl[i].replace(/\r/,"").trim();
          }
          tc=tl.join("\n");
          tc= tc.split("Input").slice(1);
          console.log("after slicing")
          console.log(tc)
          try {
            tc.forEach((ct)=>{
              const temp = ct.split("Output");
        ...
        //   console.log("Number of test files:", testarray.length);
        // }
        ...
        const problem = {
          ...
          year:req.body.Year,
          company_tags:req.body.Companies,
